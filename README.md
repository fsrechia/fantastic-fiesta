...
onsider a string containing lowercase characters from the Portuguese alphabet. You can perform two types of operations on this string:
...
Signals and Slots can only be connected once (using the same sender and receiver).

It is possible to clone an object of class QObject.

When a QWidget is inserted inside a QLayout, the QLayout becomes a parent of the QWidget.

qmake is used to compile the project.

Qt Resource System is used for storing binary files in the application's executable.

QString uses a 16-bit system for saving characters.

Every class that inherits from QObject must have the Q_OBJECT macro.

The code will not compile if you implement a Signal in the header file.

qDebug is not needed to debug classes.